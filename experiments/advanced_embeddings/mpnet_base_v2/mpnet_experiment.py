#!/usr/bin/env python3
"""
Advanced Embedding Models Experiment: all-mpnet-base-v2
===============================================

Tests the performance of all-mpnet-base-v2 (768D) vs baseline all-MiniLM-L6-v2 (384D)
for SDG text classification using cosine similarity with global thresholds.

Model Details:
- all-mpnet-base-v2: 768 dimensions, state-of-the-art sentence transformer
- Best overall model in sentence-transformers library
...
class MPNetSDGClassifier:
    def __init__(self):
        print("="*60)
        print("ADVANCED EMBEDDING EXPERIMENT: ALL-MPNET-BASE-V2")
        print("="*60)
    ...
    def load_models(self):
        """Load both MPNet and baseline models for comparison."""
        print("Loading embedding models...")
        print(f"Target model: {self.model_name} (768D)")
        print(f"Baseline model: {self.baseline_model} (384D)")
        
        # Load MPNet model
        print("Loading MPNet model...")
        self.mpnet_model = SentenceTransformer(self.model_name)
        print(f"MPNet model loaded: {self.mpnet_model.get_sentence_embedding_dimension()}D embeddings")
    ...
    def create_embeddings(self, model, texts, batch_size=32):
    ...
    def run_experiment(self):
        """Run the complete MPNet vs baseline comparison experiment."""
        ...
        # Test MPNet model
        print(f"\n{'='*60}")
        print(f"TESTING: {self.model_name.upper()}")
        print(f"{'='*60}")
        
        # Create embeddings
        print("Creating text embeddings...")
        mpnet_text_embeddings = self.create_embeddings(self.mpnet_model, texts)
        
        print("Creating SDG embeddings...")
        sdg_texts = [self.sdg_descriptions[i] for i in range(1, 18)]
        mpnet_sdg_embeddings = self.create_embeddings(self.mpnet_model, sdg_texts)
        
        # Calculate similarities
        print("Calculating similarities...")
        mpnet_similarities = self.calculate_similarities(mpnet_text_embeddings, mpnet_sdg_embeddings)
        
        # Apply thresholds
        print("Applying global thresholds...")
        mpnet_predictions, mpnet_stats = self.apply_global_thresholds(mpnet_similarities)
        
        # Evaluate
        mpnet_results = self.evaluate_performance(mpnet_predictions, true_labels, self.model_name)
        mpnet_results['assignment_stats'] = mpnet_stats
        ...
        # Create embeddings
        print("Creating text embeddings...")
        baseline_text_embeddings = self.create_embeddings(self.baseline_model_obj, texts)
        
        print("Creating SDG embeddings...")
        baseline_sdg_embeddings = self.create_embeddings(self.baseline_model_obj, sdg_texts)
    ...
    def compare_results(self, results):
        """Compare MPNet vs baseline results."""
        print(f"\n{'='*60}")
        ...
        print(f"\nMetric                    {self.model_name:<20} {self.baseline_model:<20} Improvement")
        print("-" * 80)
        
        metrics = ['f1_score', 'precision', 'recall', 'coverage', 'avg_labels_per_text']
        for metric in metrics:
            mpnet_val = mpnet[metric]
            baseline_val = baseline[metric]
            improvement = ((mpnet_val - baseline_val) / baseline_val) * 100
            
            print(f"{metric:<25} {mpnet_val:<20.4f} {baseline_val:<20.4f} {improvement:+6.1f}%")
        
        # Overall winner
        f1_improvement = ((mpnet['f1_score'] - baseline['f1_score']) / baseline['f1_score']) * 100
        winner = "MPNet" if mpnet['f1_score'] > baseline['f1_score'] else "Baseline"
    ...
    def save_results(self, results):
        ...
        results_path = f'results/mpnet_experiment_results_{timestamp}.json'
        
        # Add experiment metadata
        experiment_data = {
            'experiment': 'advanced_embeddings_mpnet',
            'timestamp': timestamp,
            'configuration': {
...
def main():
    """Run the MPNet embedding experiment."""
    classifier = MPNetSDGClassifier()
