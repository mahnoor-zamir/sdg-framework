#!/usr/bin/env python3
"""
Advanced Embedding Models Experiment: intfloat/e5-large-v2
=========================================================

Tests the performance of intfloat/e5-large-v2 (1024D) vs baseline all-MiniLM-L6-v2 (384D)
for SDG text classification using cosine similarity with global thresholds.

Model Details:
- intfloat/e5-large-v2: 1024 dimensions, state-of-the-art embedding model
- E5 series models are among the best performing on MTEB benchmark
...
class E5SDGClassifier:
    def __init__(self):
        print("="*60)
        print("ADVANCED EMBEDDING EXPERIMENT: INTFLOAT/E5-LARGE-V2")
        print("="*60)
    ...
    def load_models(self):
        """Load both E5 and baseline models for comparison."""
        print("Loading embedding models...")
        print(f"Target model: {self.model_name} (1024D)")
        print(f"Baseline model: {self.baseline_model} (384D)")
        
        # Load E5 model
        print("Loading E5 model (this may take a while for first download)...")
        self.e5_model = SentenceTransformer(self.model_name)
        print(f"E5 model loaded: {self.e5_model.get_sentence_embedding_dimension()}D embeddings")
    ...
    def prepare_e5_texts(self, texts):
        """Prepare texts for E5 model with proper prefixes."""
        # E5 models work better with query prefixes
        return [f"query: {text}" for text in texts]
        
    def create_embeddings(self, model, texts, batch_size=16, is_e5=False):
        """Create embeddings for texts using specified model."""
        # Prepare texts for E5 if needed
        if is_e5:
            texts = self.prepare_e5_texts(texts)
    ...
    def run_experiment(self):
        """Run the complete E5 vs baseline comparison experiment."""
        ...
        # Test E5 model
        print(f"\n{'='*60}")
        print(f"TESTING: INTFLOAT/E5-LARGE-V2")
        print(f"{'='*60}")
        
        # Create embeddings (with E5 prefixes)
        print("Creating text embeddings...")
        e5_text_embeddings = self.create_embeddings(self.e5_model, texts, batch_size=16, is_e5=True)
        
        print("Creating SDG embeddings...")
        sdg_texts = [self.sdg_descriptions[i] for i in range(1, 18)]
        e5_sdg_embeddings = self.create_embeddings(self.e5_model, sdg_texts, batch_size=17, is_e5=True)
        
        # Calculate similarities
        print("Calculating similarities...")
        e5_similarities = self.calculate_similarities(e5_text_embeddings, e5_sdg_embeddings)
        
        # Apply thresholds
        print("Applying global thresholds...")
        e5_predictions, e5_stats = self.apply_global_thresholds(e5_similarities)
        
        # Evaluate
        e5_results = self.evaluate_performance(e5_predictions, true_labels, "e5-large-v2")
        e5_results['assignment_stats'] = e5_stats
    ...
    def compare_results(self, results):
        """Compare E5 vs baseline results."""
        print(f"\n{'='*60}")
        ...
        print(f"\nMetric                    E5-Large-v2          MiniLM-L6-v2         Improvement")
        print("-" * 80)
        ...
        for metric in metrics:
            ...
            improvement = ((e5_val - baseline_val) / baseline_val) * 100
            
            print(f"{metric:<25} {e5_val:<20.4f} {baseline_val:<20.4f} {improvement:+6.1f}%")
        
        # Overall winner
        f1_improvement = ((e5['f1_score'] - baseline['f1_score']) / baseline['f1_score']) * 100
        winner = "E5" if e5['f1_score'] > baseline['f1_score'] else "Baseline"
    ...
    def save_results(self, results):
        ...
        results_path = f'results/e5_experiment_results_{timestamp}.json'
        
        # Add experiment metadata
        experiment_data = {
            'experiment': 'advanced_embeddings_e5',
            'timestamp': timestamp,
            'configuration': {
...
def main():
    """Run the E5 embedding experiment."""
    classifier = E5SDGClassifier()
