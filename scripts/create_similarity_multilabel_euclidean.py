#!/usr/bin/env python3
"""
Create a multi-label SDG dataset using Euclidean distance for text similarity.

This script is modified from the original cosine similarity approach to use
Euclidean distance between embeddings instead. For fair comparison, we'll convert
distances to similarities by using: similarity = 1 / (1 + distance)
"""
...
def calculate_text_similarity(texts1: List[str], texts2: List[str], method='euclidean_embeddings') -> np.ndarray:
    """Calculate similarity matrix between two sets of texts using Euclidean distance."""
    
    if method == 'euclidean_embeddings':
        ...
        distance_matrix = euclidean_distances(embeddings1, embeddings2)
        
        print("Converting distances to similarities...")
        # Convert distances to similarities using: similarity = 1 / (1 + distance)
        # This ensures that smaller distances = higher similarities
        # Distance of 0 -> similarity of 1.0
        # Distance of infinity -> similarity approaches 0
        similarity_matrix = 1 / (1 + distance_matrix)
        
    elif method == 'cosine_embeddings':
        # Keep original cosine similarity for comparison
        print("Loading sentence transformer model...")
        model = SentenceTransformer('all-MiniLM-L6-v2')
...
def create_similarity_based_multilabel_dataset(
    ...
    secondary_threshold: float = 0.2,
    max_labels: int = 5,
    similarity_method: str = 'euclidean_embeddings',
    fallback_top1_threshold: float | None = None,
    fallback_top2_threshold: float | None = None,
...
def save_multilabel_dataset(df: pd.DataFrame, stats: Dict, output_dir: str, 
    ...
    json_data = {
        'metadata': {
            ...
            'total_texts': len(df),
            'creation_method': 'euclidean_distance_similarity',
            'description': f'Multi-label SDG dataset created using {similarity_method} between OSDG texts and SDG paragraph descriptions'
        },
        'statistics': stats,
...
def main():
    parser = argparse.ArgumentParser(description='Create Euclidean distance-based multi-label SDG dataset')
    ...
    parser.add_argument('--method', '-M',
                        choices=['euclidean_embeddings', 'cosine_embeddings', 'tfidf'],
                        default='euclidean_embeddings',
                        help='Similarity calculation method (default: euclidean_embeddings)')
    ...
    print("=== Euclidean Distance-Based Multi-Label SDG Dataset Creator ===")
